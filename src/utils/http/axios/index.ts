...
const transform: AxiosTransform = {
  ...
  transformResponseHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
    const encryptKey = (res.headers || {})['encrypt-key'];
    if (encryptKey) {
  ...
  beforeRequestHook: (config, options) => {
    const { apiUrl, joinParamsToUrl, formatDate, joinTime = true, encrypt } = options;
    ...
    /** 目前后台只支持en_US/zh_CN 传递错误的话后端会报错数组越界Index 1 out of bounds for length 1 */
    /** 本地获取的是zh_CN/en */
    // 语言相关
