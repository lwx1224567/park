<template>
  ...
  <div class="text-white abox" :style="aStyle">
    <div class="w-full h-full scroll-def">
      <div class="w-full h-full p-l-8 py-3 pr-3">
        <slot></slot>
      </div>
    </div>
...
<script setup name="modelRight">
  ...
  const aStyle = computed(() => {
    ...
    return {
      width: `${w}px`,
      '--borderLeftWidth': show.value ? '1px' : 0,
    };
  });
  const onClick = () => {
...
<style lang="less" scoped>
  ...
  .abox {
    ...
    &::after {
      ...
      bottom: -60%;
      width: 100%;
      border-left: var(--borderLeftWidth) solid rgb(36 101 197 / 100%);
