...
<script setup lang="ts">
  ...
  const tableData = computed(() => roomStore.getAlarmData) as any;
  // 发送告警数据更新事件
  console.log('发送告警数据更新事件', tableData.value);
  bus.emit('alarmDataUpdated', tableData.value);
  // 点击事件
  const hangleClick = (item) => {
    bus.emit('alarmDataUpdated', [item]);// 高亮机柜设备
    bus.emit('focusRoomDeviceByDeviceId', item.parkId);
    bus.emit('handleHighlight', 0);
  };
</script>
